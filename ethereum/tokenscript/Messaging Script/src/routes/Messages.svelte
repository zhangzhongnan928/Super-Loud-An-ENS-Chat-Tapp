<script lang="ts">
    ...
    import { resolveNameFromTokenId } from "../lib/nameResolver";
	...
	const loadThreads = async () => {
		try {
			...
			for (let i = 0; i < friendsList.length; i++) {
				const item = friendsList[i];
				let name = cachedNames.get(item.sendingTokenId);
				if (name == undefined || name.length == 0) {
					name = await resolveNameFromTokenId(item.sendingTokenId, token.contractAddress);
				}
			}
			...
			let list = friendsList.map((item) => {
				if (item.sendingTokenId != token.tokenId) {
					return {
						...
						tokenIdName: cachedNames.get(item.sendingTokenId),
						wrongOwner: false,
...
<div class="text-center">Send messages to Base ENS holders.</div>
